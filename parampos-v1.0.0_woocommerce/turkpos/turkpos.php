	...
	class WC_Param_Gateway extends WC_Payment_Gateway {
		...
		public function payment_fields() 
		{
			...
			if($this->installment == 'yes'){
				...
				<p class="form-row">
					<a href="#popup-content" id="inst-link">Tüm taksit oranları ve seçeneklerini görüntülemek için tıklayınız</a>
			...
			<p class="form-row form-row-last " id="cc_cvv_field">
				<label for="cc_cvv" class="">CVV&nbsp;<abbr class="required" title="required">*</abbr></label>
				<span class="woocommerce-input-wrapper">
					<input type="text" class="input-text " name="cc_cvv" id="ccpp_creditcard_cc_cid" placeholder="CVV" value="" autocomplete="off">
				</span>
			</p>
		...
		public function process_payment( $order_id ) 
		{
			...
			$transaction->gateway_params->tdmode = '3D';
			$transaction->debug("\n\n*********\n\n");
			if (!$transaction->validateTransaction()) {
				$error_message = $transaction->result_code . ' ' . $transaction->result_message;
			...
			if ($transaction->tds AND $transaction->tds_echo) {
				return array(
					'result' => 'success',
					'redirect' => $transaction->tds_echo,
				);
			}
	 	}
 	}
...
function order_complate( $orderId ) {
	...
	if($resp === 'success' && !$result['error']){
		...
		*/
		...
		$order->add_order_note('Ödeme Param POS ile tamamlandı. İşlem no: #' . $orderTransaction['id_transaction']);
