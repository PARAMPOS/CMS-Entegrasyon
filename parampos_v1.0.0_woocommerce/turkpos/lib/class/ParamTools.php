...
class ParamTools
{
	...
	public static function rwm($message, $return = false, $type = 'fail')
	{
		if ($type == 'fail' OR $type == 'error')
			$type = 'danger';
		ParamConfig::$messages [] = array('message' => $message, 'type' => $type);
		return $return;
	}
	...
	{
		$numbers = ParamTools::formatMobile($phone);
	...
	public static function formatMobile($phone)
	{
		...
		/*
		  ...
		  echo "<br/>".ParamTools::formatMobile($phone);
		  var_dump(ParamTools::isMobile($phone));
		  }
		 */
	}
	...
	public static function recursive_array_search($needle, $haystack)
	{
		foreach ($haystack as $key => $value) {
			if ($needle === $value) {
				return array($key);
			} else if (is_array($value) && $subkey = ParamTools::recursive_array_search($needle, $value)) {
